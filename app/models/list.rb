class List < ApplicationRecord
  ...
  scope :submission_am_profile,   -> { where(meridies: "AM").where.not(updated_at: Time.zone.now.beginning_of_day..Time.zone.now.end_of_day) }
  scope :submission_pm_profile,   -> { where(meridies: "PM").where.not(updated_at: Time.zone.now.beginning_of_day..Time.zone.now.end_of_day) }
end
